...
class FindOutlierDixon(BaseClassAnalytic):
    ...
    def get_appropriate_subset(self, static_n: int, whole_set: pd.DataFrame, confidence: tuple) -> None:
        test_set = whole_set[VALUES]
        # comparator = self.find_comparator(test_set, confidence[VALUE])

        # list comprehension with UDF optimized on pd.DataFrame
        # read it like: for i in range if condition is true then print
        [
            self.print_function(static_n, confidence[KEY], whole_set, test_set[i:i+static_n].sort_values(), i)
